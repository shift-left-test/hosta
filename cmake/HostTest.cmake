...
set(GOOGLETEST_DISCOVER_HOST_TESTS_SCRIPT
  ${CMAKE_ROOT}/Modules/GoogleTestAddTests.cmake
)

function(gtest_discover_host_tests TARGET)
  # Assume that enable_testing() is called
  if(NOT CMAKE_TESTING_ENABLED)
    return()
  endif()
  ...
  set(options NO_PRETTY_TYPES NO_PRETTY_VALUES)
  set(oneValueArgs PREFIX WORKING_DIRECTORY TEST_LIST DISCOVERY_TIMEOUT)
  set(multiValueArgs EXTRA_ARGS PROPERTIES)
  ...
  add_custom_command(
    ...
    COMMAND "${CMAKE_COMMAND}"
            ...
            -D "CTEST_FILE=${ctest_tests_file}"
            -D "TEST_DISCOVERY_TIMEOUT=${ARG_DISCOVERY_TIMEOUT}"
            -P "${GOOGLETEST_DISCOVER_HOST_TESTS_SCRIPT}"
    VERBATIM
  )
...
endfunction(gtest_discover_host_tests)

function(cpputest_add_host_tests TARGET)
  # Assume that enable_testing() is called
  if(NOT CMAKE_TESTING_ENABLED)
    return()
  endif()
  ...
  # CppUTest uses TEST(Group, Name) and IGNORE_TEST(Group, Name) macros.
  # Unlike Unity Fixture where RUN_TEST_CASE and IGNORE_TEST are separate macros,
  # CppUTest's (IGNORE_)?TEST regex captures both in a single pass.
  set(cpputest_test_name_regex ".*\\([ \r\n\t]*([A-Za-z_0-9]+)[ \r\n\t]*,[ \r\n\t]*([A-Za-z_0-9]+)[ \r\n\t]*\\).*")
  set(cpputest_test_type_regex "([^A-Za-z_0-9](IGNORE_)?TEST)")
  ...
  foreach(source IN LISTS sources)
    ...
    string(REGEX MATCHALL "${cpputest_test_type_regex}[ \r\n\t]*\\(([A-Za-z_0-9 ,\r\n\t]+)\\)" found_tests "${contents}")
    foreach(hit ${found_tests})
      string(STRIP ${hit} hit)
      string(REGEX REPLACE ${cpputest_test_name_regex} "\\1.\\2" cpputest_test_name ${hit})
      string(REGEX REPLACE ${cpputest_test_name_regex} "\\1" cpputest_test_group ${hit})
      string(REGEX REPLACE ${cpputest_test_name_regex} "\\2" cpputest_test_case ${hit})

      if(cpputest_test_name IN_LIST added_tests)
        continue()
      endif()

      set(ctest_test_name ${ARG_PREFIX}${cpputest_test_name})
      add_test(NAME ${ctest_test_name} COMMAND ${_output} -sg ${cpputest_test_group} -sn ${cpputest_test_case} ${ARG_EXTRA_ARGS})
      list(APPEND added_tests ${cpputest_test_name})

      if(hit MATCHES "IGNORE_TEST")
        set_tests_properties(${ctest_test_name} PROPERTIES DISABLED TRUE)
      endif()
    endforeach()
  endforeach()
endfunction(cpputest_add_host_tests)

set(CPPUTEST_DISCOVER_HOST_TESTS_SCRIPT
  ${CMAKE_CURRENT_LIST_DIR}/CppUTestAddTests.cmake
)

function(cpputest_discover_host_tests TARGET)
  # Assume that enable_testing() is called
  if(NOT CMAKE_TESTING_ENABLED)
    return()
  endif()
  ...
  add_custom_command(
    ...
    COMMAND "${CMAKE_COMMAND}"
            ...
            -D "CTEST_FILE=${ctest_tests_file}"
            -D "TEST_DISCOVERY_TIMEOUT=${ARG_DISCOVERY_TIMEOUT}"
            -P "${CPPUTEST_DISCOVER_HOST_TESTS_SCRIPT}"
    VERBATIM
  )
...
endfunction(cpputest_discover_host_tests)
