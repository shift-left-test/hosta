...
**hosta** is a CMake framework for building and running C/C++ unit tests on the host platform, even within cross-compilation environments. The core product is a set of CMake scripts (in `cmake/`) that provide `add_host_executable()`, `add_host_library()`, and test integration functions for the Unity, Google Test, and CppUTest frameworks.
...
- **HostTest.cmake** — Test integration layer. Provides `add_host_test()`, `unity_fixture_add_host_tests()`, `unity_fixture_discover_host_tests()`, `gtest_add_host_tests()`, `gtest_discover_host_tests()`, `cpputest_add_host_tests()`, `cpputest_discover_host_tests()`. Registers tests with CTest.
...
- **CppUTestAddTests.cmake** — Runtime test discovery script for CppUTest tests (invoked by CTest).
...
Test files follow the pattern `*_test.py`. The test project template in `tests/project/` contains a calculator app with both C and C++ implementations, Unity tests, Google Test tests, and CppUTest tests.

### Sample Project (`sample/`)

A reference implementation showing how to use hosta in a real project with Unity, Google Test, CppUTest, and fff (Fake Function Framework).
